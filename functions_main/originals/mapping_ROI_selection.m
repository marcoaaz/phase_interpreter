function [ROI_mask, pos_ROI] = mapping_ROI_selection(phasemap_colour, tablerank2, mask_type)
...
imshow(phasemap_colour, triplet2) %triplet required for discrete colorbar
...
if strcmp(mask_type, 'circle')
    h = drawcircle('Color', 'k', 'FaceAlpha', alpha_value);
elseif strcmp(mask_type, 'rectangle')
    h = drawrectangle('Color', 'k', 'FaceAlpha', alpha_value);
elseif strcmp(mask_type, 'free_hand')
    h = drawfreehand("Color",'k', 'FaceAlpha', alpha_value);
elseif strcmp(mask_type, 'polygon')
    h = drawpolygon("Color",'k', 'FaceAlpha', alpha_value);
end

ROI_mask = createMask(h); %logical
